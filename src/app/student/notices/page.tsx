...
export default function StudentNoticesPage() {
  ...
  const getRelevancePill = (level: string) => {
    switch (level) {
      case "HIGH":
        return {
          label: "🟢 Relevant",
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-16 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredNotices.length > 0 ? (
          filteredNotices.map((notice) => {
            const relPill = getRelevancePill(notice.relevance.relevance);
            const isRelevant = notice.relevance.relevance === "HIGH" || notice.relevance.relevance === "MEDIUM";

            return (
              <div
                key={notice.id}
                onClick={() => handleOpenNotice(notice)}
                className={`p-5 rounded-3xl border transition-all cursor-pointer flex flex-col justify-between text-left space-y-4 group ${
                  isRelevant
                    ? "bg-white border-slate-200/90 hover:border-indigo-300 shadow-xs hover:shadow-md"
                    : "bg-slate-50/70 border-slate-200/70 opacity-85 hover:opacity-100 hover:bg-white"
                }`}
              >
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${relPill.className}`}>
                        {relPill.label}
                      </span>
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                  ...
                  <span className="text-indigo-600 font-bold text-xs flex items-center gap-1 group-hover:translate-x-0.5 transition-transform">
                    <span>{isRelevant ? "Read Notice" : "Inspect Notice"}</span>
      ...
      {selectedNotice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-3xl max-w-xl w-full border border-slate-200 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 text-left flex flex-col max-h-[85vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
              {/* Relevance reason box */}
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-800">
                    Relevance Assessment: {selectedNotice.relevance.relevance}
                  </span>
                  <span className="font-bold text-slate-600">
                    Score: {selectedNotice.relevance.score}/100
                  </span>
                </div>
                <div className="space-y-1 text-[11px] text-slate-600">
                  {selectedNotice.relevance.reasons.map((r, i) => (
